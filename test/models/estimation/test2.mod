...
first_chain = rwmh_compute!(datafile="dsge1_data.csv", mcmc_replic=100000, mcmc_jscale=0.02, mcmc_chains=1, transformed_parameters = true, back_transformation = true);
new_covariance = covariance(first_chain);
second_chain = rwmh_compute!(datafile="dsge1_data.csv", transformed_covariance = Matrix(new_covariance), initial_values=first_chain.value.data[end, 1:end-1], mcmc_replic=100000, mcmc_jscale=0.15, mcmc_chains=1, transformed_parameters = true, back_transformation = true);
