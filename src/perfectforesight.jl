...
struct PerfectForesightOptions
    ...
    function PerfectForesightOptions(options::Dict{String,Any})
        ...
        new(algo, datafile, display, maxit, periods, tolf, tolx)
    end
end
...
struct PerfectForesightWs
    y::Vector{Float64}
    x::Vector{Float64}
    function PerfectForesightWs(endogenous_nbr, exogenous_nbr, periods)
...
function compute_prefect_foresight_setup(context::Context) end
