...
struct Parameters
    ...
    function Parameters()
        ...
        new(symbols, calibrations, distributions, initialvalues, mh_scale,
            ...
            posteriormedian, posteriorHPI, transfrombasic, transformtobasic)
    end
end
...
#    parameters = context.results.model_results[1].parameters
    for p in fields["params"]
        ...
        push!(parameters.transformfrombasic, get_transformfrombasic(Val(p["prior_distribution"]), p["3"], p["4"]))
        push!(parameters.transformtobasic, get_transformtobasic(Val(p["prior_distribution"]), p["3"], p["4"]))
