...
function parse_estimated_parameters!(context::Context, fields::Dict{String,Any})
    ...
    for p in fields["params"]
        ...
        elseif "var" in keys(p)
            ...
            is_endogenous(p["var"], symbol_table) && push!(parameters.parametertype, EstSDMeasurement) 
            is_exogenous(p["var"], symbol_table) && push!(parameters.parametertype, EstSDShock)
        elseif "var1" in keys(p)
            ...
            is_endogenous(p["var"], symbol_table) && push!(parameters.parametertype, EstCorrMeasurement) 
            is_exogenous(p["var"], symbol_table) && push!(parameters.parametertype, EstCorrShock)
        else
