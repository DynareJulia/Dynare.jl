...
context = @dynare "../models/GIMF/gimf_gmres" "nostrict" "nocompile"
...
periods = 12
exogenous = repeat(context.results.model_results[1].trends.exogenous_steady_state', periods)
#exogenous[2, md.exogenous_nbr] = 0.01
window = 2
y = zeros(n, periods)
tmp1 = similar(y)
tmp2 = similar(y)
c = context.results.model_results[1].trends.endogenous_steady_state
x = exogenous .- transpose(context.results.model_results[1].trends.exogenous_steady_state)
simul_first_order_1!(y, zeros(n), A, B, x, window, periods, tmp1, tmp2)
...
initialvalues = y[1:n]
@show steadystate[370]
@show y[[370, 370 + md.endogenous_nbr, 370 + 2 * md.endogenous_nbr]]
