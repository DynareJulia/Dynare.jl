///////
// TO BE RUN WITH context = @dynare "irbc" "-DN=<nbr of countries>" "-D;
///////
///////
// IRBC model from
...
model;
  @#for j in 1:N
    exp(lambda)*(1 + phi*(k_@{j}/k_@{j}(-1) - 1))
      = beta*exp(lambda(+1))*(exp(a_@{j}(+1))*kappa*A*k_@{j}^(kappa - 1)
  ...
  @#endfor
  =
  (exp(lambda)/t_1)^(-gamma_1) + k_1 - (1 - delta)*k_1(-1) + (phi/2)*k_1(-1)*(k_1/k_1(-1) - 1)^2
  @#for j in 2:N
  + (exp(lambda)/t_@{j})^(-gamma_@{j}) + k_@{j} - (1 - delta)*k_@{j}(-1)
